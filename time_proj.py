...
class HyTE(Model):
    ...
    def getOneHot(self, start_data, end_data, num_class):
        ...
        for i, ele in enumerate(start_data):
            if end_data[i] >= start_data[i]:
                temp[i, start_data[i] : end_data[i] + 1] = 1 / (
                    end_data[i] + 1 - start_data[i]
                )
            else:
                pdb.set_trace()
        return temp
    ...
    def load_data(self):
        ...
        train_triples = []
        self.start_time, self.end_time, self.num_class = (
            ddict(dict),
            ddict(dict),
            ddict(dict),
        )
        triple_time, entity_time = dict(), dict()
        self.inp_idx, self.start_idx, self.end_idx, self.labels = (
            ddict(list),
            ddict(list),
            ddict(list),
            ddict(list),
        ...
        (
            self.inp_idx["triple"],
            self.start_idx["triple"],
            self.end_idx["triple"],
        ) = self.create_id_labels(triple_time, "triple")
        # pdb.set_trace()
        for i, ele in enumerate(self.inp_idx["entity"]):
            if self.start_idx["entity"][i] > self.end_idx["entity"][i]:
                print(
                    self.inp_idx["entity"][i],
                    self.start_idx["entity"][i],
        ...
        for i in range(len(posh)):
            if self.start_idx["triple"][i] < self.end_idx["triple"][i]:
                for j in range(
                    self.start_idx["triple"][i] + 1, self.end_idx["triple"][i] + 1
                ):
                    head.append(posh[i])
                    rel.append(rela[i])
                    tail.append(post[i])
    ...
    def calculated_score_for_positive_elements(
        self, t, epoch, f_valid, eval_mode="valid"
    ):
        ...
        pos_head = sess.run(
            self.pos,
            feed_dict={
                self.pos_head: np.array([t[0]]).reshape(-1, 1),
    ...
    def time_projection(self, data, t):
        inner_prod = tf.tile(
            tf.expand_dims(tf.reduce_sum(data * t, axis=1), axis=1), [1, self.p.inp_dim]
        )
        prod = t * inner_prod
        data = data - prod
        return data
    ...
    def add_model(self):
        ...
        def f_test():
            def head_tail_query():
                def f_head():
                    ...
                    pos_h_e = transE_in
                    pos_t_e = tf.reshape(
                        tf.tile(e2, [self.max_ent]), (self.max_ent, transE_in_dim)
                    )
                ...
                def f_tail():
                    ...
                    pos_h_e = tf.reshape(
                        tf.tile(e1, [self.max_ent]), (self.max_ent, transE_in_dim)
                    )
                    pos_t_e = transE_in
                    return pos_h_e, pos_t_e
                ...
                pos_r_e = tf.reshape(
                    tf.tile(r, [self.max_ent]), (self.max_ent, transE_in_dim)
                )
                return pos_h_e, pos_t_e, pos_r_e
            ...
            def rel_query():
                ...
                e2 = tf.squeeze(tf.nn.embedding_lookup(transE_in, self.pos_tail))
                pos_h_e = tf.reshape(
                    tf.tile(e1, [self.max_rel]), (self.max_rel, transE_in_dim)
                )
                pos_t_e = tf.reshape(
                    tf.tile(e2, [self.max_rel]), (self.max_rel, transE_in_dim)
                )
                pos_r_e = self.rel_embeddings
                return pos_h_e, pos_t_e, pos_r_e

            pos_h_e, pos_t_e, pos_r_e = tf.cond(
                self.query_mode > query_type, head_tail_query, rel_query
            )
            return pos_h_e, pos_t_e, pos_r_e
        ...
        if self.p.L1_flag:
            pos = tf.reduce_sum(
                abs(pos_h_e_t_1 + pos_r_e_t_1 - pos_t_e_t_1), 1, keep_dims=True
            )
            neg = tf.reduce_sum(
                abs(neg_h_e_t_1 + pos_r_e_t_1 - neg_t_e_t_1), 1, keep_dims=True
            )
            # self.predict = pos
        else:
            pos = tf.reduce_sum(
                (pos_h_e_t_1 + pos_r_e_t_1 - pos_t_e_t_1) ** 2, 1, keep_dims=True
            )
            neg = tf.reduce_sum(
                (neg_h_e_t_1 + pos_r_e_t_1 - neg_t_e_t_1) ** 2, 1, keep_dims=True
            )
            # self.predict = pos
    ...
    def add_optimizer(self, loss):
        ...
        time_normalizer = tf.assign(
            self.time_embeddings, tf.nn.l2_normalize(self.time_embeddings, dim=1)
        )
        return train_op
        ...
        if not self.p.onlyTest:
            ...
            for epoch in range(self.p.max_epochs):
                ...
                if epoch % self.p.test_freq == 0 and epoch != 0:
                    ...
                    fileout_head = open(
                        save_dir_results + "/valid_head_pred_{}.txt".format(epoch), "w"
                    )
                    fileout_tail = open(
                        save_dir_results + "/valid_tail_pred_{}.txt".format(epoch), "w"
                    )
                    fileout_rel = open(
                        save_dir_results + "/valid_rel_pred_{}.txt".format(epoch), "w"
                    )
                    for i, t in enumerate(validation_data):
                        score = self.calculated_score_for_positive_elements(
                            t, epoch, f_valid, "valid"
                        )
                        if score:
                            fileout_head.write(
                                " ".join([str(x) for x in score[0]]) + "\n"
                            )
                            fileout_tail.write(
                                " ".join([str(x) for x in score[1]]) + "\n"
                            )
                            fileout_rel.write(
                                " ".join([str(x) for x in score[2]]) + "\n"
                            )
            ...
            fileout_head = open(
                save_dir_results
                + "/test_head_pred_{}.txt".format(self.p.restore_epoch),
                "w",
            )
            fileout_tail = open(
                save_dir_results
                + "/test_tail_pred_{}.txt".format(self.p.restore_epoch),
                "w",
            )
            fileout_rel = open(
                save_dir_results + "/test_rel_pred_{}.txt".format(self.p.restore_epoch),
                "w",
            )
            for i, t in enumerate(test_data):
                score = self.calculated_score_for_positive_elements(
                    t, self.p.restore_epoch, f_test, "test"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-test_freq", dest="test_freq", default=25, type=int, help="Batch size"
    )
    ...
    parser.add_argument(
        "-lam_1", dest="lambda_1", default=0.5, type=float, help="transE weight"
    )
    parser.add_argument(
        "-lam_2", dest="lambda_2", default=0.25, type=float, help="entitty loss weight"
    )
    ...
    parser.add_argument(
        ...
        dest="L1_flag",
        action="store_false",
        help="Hidden state dimension of FC layer",
    )
    parser.add_argument(
        "-onlytransE",
        dest="onlytransE",
    ...
    parser.add_argument(
        ...
        dest="onlyTest",
        action="store_true",
        help="Evaluate model for test data",
    )
    parser.add_argument(
        "-restore",
        dest="restore",
    ...
    args.relation2id = (
        "data/" + args.data_type + "/" + args.version + "/relation2id.txt"
    )
